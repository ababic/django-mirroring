...
def assert_compatible_release(actual: str, pinned: str, *, label: str) -> None:
    ...
    if actual_v[:2] != pinned_v[:2]:
        raise CommandError(
            f"{label} version {actual!r} is incompatible with pinned {pinned} (expected {pinned_v[0]}.{pinned_v[1]}.x)."
        )
    if actual_v < pinned_v:
        raise CommandError(f"{label} version {actual!r} is older than pinned minimum {pinned}.")
