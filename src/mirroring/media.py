...
def iter_filefield_media_refs(
    *,
    ...
    for model, label, all_file_fields in _iter_concrete_filefield_models():
        ...
        if (
            only_anonymise
            and label not in anonymise_models
            and not any(f"{label}.{field.name.lower()}" in anonymise_fields for field in all_file_fields)
        ):
            continue
...
def load_extra_collectors(dotted_paths: Iterable[str] | None = None) -> list[ExtraCollector]:
    ...
    paths = (
        list(dotted_paths)
        if dotted_paths is not None
        else list(getattr(settings, "MEDIA_SYNC_EXTRA_COLLECTORS", []) or [])
    )
